...
class Http2Connection internal constructor(builder: Builder) : Closeable {
    ...
    private val writerExecutor = ScheduledThreadPoolExecutor(
        1,
    ...
    init {
        if (builder.pingIntervalMillis != 0) {
            writerExecutor.scheduleAtFixedRate(
                ...
                builder.pingIntervalMillis.toLong(),
                builder.pingIntervalMillis.toLong(),
                MILLISECONDS
            )
        }
        ...
        override fun headers(
            ...
            synchronized(this@Http2Connection) {
                ...
                if (stream == null) {
                    ...
                    val newStream =
                        Http2Stream(streamId, this@Http2Connection, false, inFinished, headers)
                    ...
                    listenerExecutor.execute("OkHttp $connectionName stream $streamId") {
                        try {
                            ...
                            Platform.get().log(
                                INFO,
                                "Http2Connection.Listener failure for $connectionName",
                                e
                            )
                            ignoreIoExceptions {
    ...
    companion object {
         ...
         */
        private val listenerExecutor = ThreadPoolExecutor(
            0,
